...
## What's new — Dashboard v2 (May 2026)

Re-imagined `/app/dashboard` as a widescreen, edge-to-edge, operator-grade
console. See [`phase3-fullstack/docs/DASHBOARD_V2.md`](phase3-fullstack/docs/DASHBOARD_V2.md)
for full architecture and [`phase3-fullstack/docs/MCP_TOOLS.md`](phase3-fullstack/docs/MCP_TOOLS.md)
for the eight MCP tools the LLM advisor calls.

- **AI inference console** — RF-DETR vs YOLO 26n side-by-side detector battle
  with mAP / size / latency bars, ByteTrack live FPS sparkline, LightGBM
  per-corridor demand deltas, Webster delay-reduction bar, Claude-MCP tool
  palette. One panel, six models, every metric live.
- **Embedded AI advisor** — full-width chat panel at the bottom of the
  dashboard. Same Claude session as the floating drawer. Suggested prompts,
  tool-call chips on every assistant message coloured by tool family.
- **New MCP tool `get_typical_day_gmaps`** — exposes the Google-typical-day
  congestion grid (4 corridors × 48 half-hour bins) so the advisor can compare
  "now vs typical" for any hour. JSON artefact at
  `data/research/gmaps/typical_2026-04-26.json`, generated by
  `tools/build_typical_day_json.py`.
- **Fixes** — `fusion.py` NULL `crossings_pce_in_bin` guard;
  `POST /api/video/restart` so a dashboard refresh re-anchors the signal cycle
  to video frame 0; SPA-fallback static-files subclass so `/app/<deep-link>`
  survives a hard refresh.
- **Stack**: Geist + Geist Mono fonts, `OperatorTopBar` replaces the global
  nav on `/dashboard` only.

## What's new — production-readiness upgrade
